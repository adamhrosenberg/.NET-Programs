...
namespace SS
{
    ...
    public abstract class AbstractSpreadsheet
    {
        ...
        {
            visited.Add(name); //add this one to the list of visited
            foreach (String n in GetDirectDependents(name)) //look through all direct dependents of name
            {
                if (n.Equals(start))//name is same as start -> theyre refferencing eachother
                {
                    throw new CircularException();
                }
                else if (!visited.Contains(n))//if string n is not in there continue visiting
                {
                    Visit(start, n, visited, changed);
                }
            }
            changed.AddFirst(name);//Add new header of linked list -> gets out of recursion
        }
